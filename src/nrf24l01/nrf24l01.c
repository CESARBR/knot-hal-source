...
/* Hold pipe0 address pointer for address recovery it */
bool m_bpipe0 = false;
uint8_t	m_pipe0_address[NRF24_ADDR_WIDTHS];
...
static inline int8_t nrf24reg_read(int8_t spi_fd, uint8_t reg)
{
	...
	spi_bus_transfer(spi_fd, &reg, BYTE_SIZE, &value, BYTE_SIZE);
	return (int8_t)value;
}
					...
					void *pd, uint16_t len)
{
	...
	reg = NRF24_R_REGISTER(reg);
	spi_bus_transfer(spi_fd, &reg, BYTE_SIZE, pd, len);
}
 ...
 */
static inline void nrf24reg_write(int8_t spi_fd, uint8_t reg,
							uint8_t value)
{
...
{
	spi_bus_transfer(spi_fd, NULL, 0, &cmd, BYTE_SIZE);
...
static inline int8_t command_data(int8_t spi_fd, uint8_t cmd, void *pd,
						uint16_t len)
{
	spi_bus_transfer(spi_fd, &cmd, BYTE_SIZE, pd, len);
	/* Return device status register */
...
/* Set address in pipe */
static void set_address_pipe(int8_t spi_fd, uint8_t reg,
						uint8_t *pipe_addr)
{
	uint8_t addr[NRF24_ADDR_WIDTHS];
	int8_t len;
...
/* Get address of pipe */
static void get_address_pipe(int8_t spi_fd, uint8_t pipe,
								uint8_t *pipe_addr)
{
						...
						uint8_t *pipe_addr)
{
	...
	if (!(value & NRF24_EN_RXADDR_PIPE(pipe))) {
		nrf24reg_write(spi_fd,
					NRF24_EN_RXADDR, value | NRF24_EN_RXADDR_PIPE(pipe));
		set_address_pipe(spi_fd, NRF24_RX_ADDR_PIPE(pipe), pipe_addr);
		/* Hold pipe0 address whether case on */
		if(pipe == NRF24_PIPE0_ADDR){
			memcpy(m_pipe0_address, pipe_addr, sizeof(m_pipe0_address));
			m_bpipe0 = true;
		}
		else
		/* Set the pipe1 address on case disabled to can open another
		 * one pipe above
		 */
		if(pipe > NRF24_PIPE1_ADDR && !(value & NRF24_EN_RXADDR_P1))
...
int8_t nrf24l01_close_pipe(int8_t spi_fd, int8_t pipe)
{
	...
	if (nrf24reg_read(spi_fd,
					NRF24_EN_RXADDR) & NRF24_EN_RXADDR_PIPE(pipe)) {
		/*
...
{
	uint8_t pipe_addr[NRF24_ADDR_WIDTHS];
...
int8_t nrf24l01_set_prx(int8_t spi_fd)
{
	 ...
	 */
	if(m_bpipe0)
		set_address_pipe(spi_fd, NRF24_RX_ADDR_P0, m_pipe0_address);
	else
		nrf24reg_write(spi_fd, NRF24_EN_RXADDR,
			nrf24reg_read(spi_fd, NRF24_EN_RXADDR)
						& ~NRF24_EN_RXADDR_P0);
...
int8_t nrf24l01_prx_data(int8_t spi_fd, void *pdata, uint16_t len)
{
	...
	command_data(spi_fd, NRF24_R_RX_PL_WID, &rxlen, BYTE_SIZE);
