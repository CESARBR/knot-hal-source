...
static int nrf24l01_ioctl(int spi_fd, int cmd, void *arg)
{
	...
	case NRF24_CMD_SET_PIPE:
		par.addr = (struct addr_pipe*)arg;
		err = nrf24l01_open_pipe(spi_fd, par.addr->pipe, par.addr->aa);
		break;
	...
	case NRF24_CMD_SET_CHANNEL:
		par.ch = (struct channel*)arg;
		err = nrf24l01_set_channel(spi_fd, par.ch->value, par.ch->ack);
		break;
